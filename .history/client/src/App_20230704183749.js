...
const App = () => {
  ...
  const [filteredMarkersByTags, setFilteredMarkersByTags] =
    useState(filteredMarkers);

  const filterMarkersByTags = (tagInput) => {
    if (!tagInput) {
      setFilteredMarkersByTags(filteredMarkers); // If no tag input is given, display all entries
    } else {
      const lowercaseTagInput = tagInput.toLowerCase(); // Convert tag input to lowercase for case-insensitive comparison
      const filteredByTags = filteredMarkers.filter(({ tag }) =>
        tag.toLowerCase().includes(lowercaseTagInput)
      );
      setFilteredMarkersByTags(filteredByTags);
    }
  };

  const filteredMarkersTags = filterMarkersByTags(filteredMarkersToShow);

  const renderedMarkers = filteredMarkersTags.map(
    (
