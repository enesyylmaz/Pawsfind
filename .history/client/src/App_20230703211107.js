...
const App = () => {
  ...
  const filterMarkersByDate = (markers, selectedOption) => {
    const currentDate = new Date();
    const filteredMarkers = markers.filter(({ date }) => {
      ...
      if (selectedOption === "1day") {
        ...
        return markerDate >= threeDaysAgo && markerDate <= currentDate;
      } else if (selectedOption === "1week") {
        const oneWeekAgo = new Date();
        oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
        return markerDate >= oneWeekAgo && markerDate <= currentDate;
      }
      return true; // Default case: show all markers if option not matched
    });

    return filteredMarkers;
  };

  const handleDisplayyTime = (event) => {
    const selectedOption = event.target.value;
    const filteredMarkers = filterMarkersByDate(markers, selectedOption);
    ...
    <main>
      ...
      <div className="map-container" ref={mapRef}>
        ...
        >
          {filteredMarkers.map(
            (
              { lat, lng, name, variant, email, description, image, date },
              index
            ) => {
