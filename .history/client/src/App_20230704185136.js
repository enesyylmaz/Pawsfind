...
const App = () => {
  ...
  const filterMarkersUsingTags = (tags) => {
    if (!tags || tags.length === 0) {
      // If no tags are provided, return all markers
      return filteredMarkers;
    } else {
      // Filter markers based on the provided tags
      return filteredMarkers.filter(({ tag }) => {
        // Check if any of the tags in the marker match the provided tags
        return tag.some((markerTag) => tags.includes(markerTag));
      });
    }
  };

  const filteredMarkersByTags = filterMarkersUsingTags(filteredMarkersToShow);

  useEffect(() => {
    setFilteredMarkersByTags(filteredMarkersToShow);
  }, [filteredMarkersToShow]);
