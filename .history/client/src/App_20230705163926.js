...
const App = () => {
  ...
  const handleTagInputChangeSearch = (value) => {
    const lowercaseValue = value.toLowerCase();
    setTagInputSearch(lowercaseValue);
  };
  ...
  const removeTagSearch = (index) => {
    setTagsSearch(tagsSearch.filter((el, i) => i !== index));
  };
  ...
  const clusterMarkers = () => {
    const markers = coordinateData.map((markerData) => ({
      position: { lat: markerData.lat, lng: markerData.lng },
    }));

    const clusters = [];
    const clusterSize = 0.01; // Adjust the cluster size as per your needs

    markers.forEach((marker) => {
      let addedToCluster = false;
      clusters.forEach((cluster) => {
        const distance = Math.sqrt(
          (cluster.position.lat - marker.position.lat) ** 2 +
            (cluster.position.lng - marker.position.lng) ** 2
        );
  ...
  const renderedClusters = clusterMarkers().map((cluster, index) => (
    <Marker
      key={index}
      position={cluster.position}
      label={cluster.markers.length.toString()}
    />
  ));
  ...
  const filterMarkersUsingTags = (tags) => {
    if (!tags || tags.length === 0) {
      return filteredMarkersToShow;
    } else {
      return filteredMarkersToShow.filter(({ tag }) => {
        console.log(tags);
        console.log(tag);
        return tags.every((taga) => tag && tag.includes(taga));
      });
    }
  };

  const filteredMarkersByTags = filterMarkersUsingTags(tagsSearch);
    ...
    <main>
      ...
      <div>
        <div className="tag-field">
          {tagsSearch.map((tagSearch, index) => (
            <div className="tag-item" key={index}>
              <span>{tagSearch}</span>
              <span className="close" onClick={() => removeTagSearch(index)}>
                &times;
              </span>
            </div>
          ))}
      ...
      {highlighted && (
        <div className="popup">
          <div className="popup-content">
            ...
            <br />
            <div>
              <div className="tag2-field">
                {highlighted.markerTags &&
                  highlighted.markerTags.map((tagSearch, index) => (
                    <div className="tag-item" key={index}>
                      <span>{tagSearch}</span>
                    </div>
                  ))}
