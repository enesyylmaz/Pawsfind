...
const App = () => {
  ...
  const handleDisplay = (event) => {
    setDisplay(event.target.value === "all" ? "" : event.target.value);
  };
  ...
  const filteredMarkers =
    display === "all"
      ? coordinateData
      : coordinateData.filter(
          (data) =>
            data.variant === (display === "found" ? "variant2" : "variant1")
        );
  ...
  return (
    <main>
      ...
      <div className="map-container" ref={mapContainerRef}>
        ...
        >
          {filteredMarkers.map(({ lat, lng, name, variant }, index) => (
            <Marker
              key={index}
              lat={lat}
              lng={lng}
              ...
              >
                <div className="grid place-items-center">
                  ...
                  <div className="input-field w-3/4 h-16 p-2 m-2 border-2 border-gray-300 rounded-lg flex items-center justify-between">
                    <label htmlFor="imageUpload" className="browse-label">
                      {image ? (
