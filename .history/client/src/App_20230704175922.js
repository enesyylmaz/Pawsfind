...
const App = () => {
  ...
  const [tagsSear, setTagsSear] = useState([]);
  const [tagInputSear, setTagInputSear] = useState("");

  const handleTagInputChangeSear = (value) => {
    setTagInputSear(value);
  };

  const handleKeyDownSear = (e) => {
    if (e.key !== "Enter") return;
    const value = e.target.value;
    if (!value.trim()) return;
    setTagsSear([...tags, value]);
    setTagInputSear("");
  };

  const removeTagSear = (index) => {
    setTagsSear(tagsSear.filter((el, i) => i !== index));
  };
  ...
  const onGoogleApiLoaded = ({ map, maps }) => {
    ...
    markers = filteredMarkersToShow.map(
      (
